...
def validate_filename(filename: str) -> bool:
    ...
    if '..' in filename:
        raise SecurityValidationError(
            f"Filename '{filename}' contains path traversal pattern '..' "
        )
    
    # Prevent directory separators
    if '/' in filename or '\\' in filename:
        raise SecurityValidationError(
            f"Filename '{filename}' cannot contain directory separators"
        )
    
    # Prevent hidden files
    if filename.startswith('.'):
        raise SecurityValidationError(
            f"Filename '{filename}' cannot start with a dot (hidden file)"
        )
    
    # Prevent null bytes
    if '\0' in filename:
        raise SecurityValidationError(
            f"Filename '{filename}' contains null bytes"
        )
    
    # Check length (prevent DOS via extremely long filenames)
    if len(filename) > 255:
        raise SecurityValidationError(
            f"Filename '{filename}' is too long (max 255 characters)"
        )
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            if len(value) > max_string_length:
                raise SecurityValidationError(
                    f"Value for key '{key}' is too long "
                    f"(max {max_string_length} characters)"
                )
...
def validate_file_type(
    ...
    if not ext:
        raise SecurityValidationError(
            f"{file_type} '{filename}' has no extension"
        )
